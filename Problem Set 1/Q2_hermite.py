...
def cubic_hermite_spline(z,x,y,y_prime):
    assert len(x) == len(y), "x and y must have the same dimensions, Dave."
    n = len(x)
    for i in range(n-1):
        if (x[i]<=z):
            if(z<=x[i+1]):
                v = np.array([y[i],y[i+1],y_prime[i],y_prime[i+1]])
                mat = np.array(
                    [
                        [x[i]**3,x[i]**2,x[i],1],
...
dv_dt = dv_dt/1000 # convert from [mv/K] to [V/K]

func = lambda z: cubic_hermite_spline(z,t,v,dv_dt)

# Ask the user for an input temperature and calculate the diode voltage
# at that temperature using the hermite cubic interpolation
...
if(plot_interp == 'y' or plot_derivative == 'y'):
    x = np.linspace(t[0],t[len(t)-1],100000)
...
if(plot_interp == 'y'):
    ...
    plt.title("Hermite Cubic Interpolation")

    # Plot the hermite cublic spline interpolation
