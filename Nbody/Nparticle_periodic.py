...
# dt = 0.001 and n =2**7 for clustering
...
if(save_data):
    x_save = np.zeros([niter,npt])
    y_save = np.zeros([niter,npt])
    z_save = np.zeros([niter,npt])
...
if(verbose):
    ...
    if(save_data):
        ...
        print(">Positional data of every particle will be saved every %dth iteration"%save_iter)
