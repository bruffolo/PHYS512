...
def calc_grad(p,l):
    ...
    dx = p0/1000#np.array([0.00002,0.00002,0.0001,2e-12,0.00096])
    grad = np.zeros([y0.size,p.size])

    grad[:,0]= ( get_cmb(p+[dx[0],0,0,0,0]) - y0 )/dx[0]
...
p0 = np.asarray([45,0.04,0.08,2e-9,1.0]) #np.asarray([45,0.04,0.1,2e-9,0.86])
